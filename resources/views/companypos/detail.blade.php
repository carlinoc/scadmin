            ...
            .then(result => {
                if(result.status=="success") {
                    ...
                    for($i = 0; $i < _dsExpense.length; $i++) {
                        ...
                        addRowExpense(rw.expenseDate, rw.expense, rw.id, $i, rw.expenseType, who, rw.staffPayType);
                    }
                    _lExpense.html('S/ ' + totalExpense);

                    setTimeout(function(){
                        _totalDif = _totalDif - totalExpense;
                        _lTotal.html('S/ ' + _totalDif);    
                    }, 300);
                }
            });   
        }

        function addRowExpense(vdate, vincome, vid, vindex, vexpenseType, vwho, vstaffPayType) {
